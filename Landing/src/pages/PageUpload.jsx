...
export default function PageUpload({ setPage }) {
  ...
  const uploadFile = async (file) => {
    ...
    try {
      ...
      // The backend derives plan server-side from the verified auth token
      // (never trusts a client-supplied plan), so this request must carry
      // the user's Firebase ID token or every upload gets rejected with
      // "Missing auth token" — including for logged-in users.
      const headers = {};
      if (user) {
    ...
    {currentProjectName && (
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>
          {/* File Preview */}
          <ExpandableCard
            expanded={previewExpand.expanded}
            onClose={previewExpand.close}
            className="card"
          ...
          >
            <div className="card-title">
              ...
              <ExpandButton
                expanded={previewExpand.expanded}
                onClick={previewExpand.toggle}
                style={activeSession?.preview ? {} : { marginLeft: "auto" }}
              />
            </div>

            {activeSession?.preview ? (
              <div style={{ overflow: "hidden", width: "100%", flex: previewExpand.expanded ? 1 : undefined, display: previewExpand.expanded ? "flex" : "block", flexDirection: "column", minHeight: 0 }}>
                <div style={{ overflowX: "auto", overflowY: "auto", maxHeight: previewExpand.expanded ? "none" : 340, flex: previewExpand.expanded ? 1 : undefined, width: "100%", minHeight: 0 }}>
