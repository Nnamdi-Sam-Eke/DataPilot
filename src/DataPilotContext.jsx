...
export function DataPilotProvider({ children }) {
  ...
  // ── Live-sync the active session's workspace snapshot ──────────────────
  // saveWorkspaceToSession only used to run on switchSession/addSession/
  // removeSession — so sessions[activeIdx].workspace (which Dashboard reads
  // trainedModels/accuracy from) stayed frozen at whatever it was when the
  // session became active. Training a model, running insights, etc. updated
  // the live `trainedModels` state but never touched the copy sitting in
  // `sessions`, so the Dashboard KPI cards only picked up new models after
  // switching away from and back to that session (or a reload). Mirror the
  // live workspace into the session slot on every change instead.
  useEffect(() => {
    if (activeIdx === null || activeIdx === undefined) return;
    saveWorkspaceToSession(activeIdx, activeWorkspace);
  }, [activeIdx, activeWorkspace, saveWorkspaceToSession]);
