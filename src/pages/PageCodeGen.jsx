    ...
    cleanOpLog.slice().reverse().forEach(op => {
      if (op.type === "fill") {
        ...
        if (col && strat) {
          ...
          else if (strat === "ffill")  lines.push(`df['${col}'].fillna(method='ffill', inplace=True)`);
          else if (strat === "bfill")  lines.push(`df['${col}'].fillna(method='bfill', inplace=True)`);
...
function buildMarkdown({ fileName, columns, summary, rowCount,
  ...
  lines.push("---");
  lines.push("*Generated by [DataPilot](http://localhost:5173)*");
